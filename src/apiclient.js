const baseURL = typeof window !== 'undefined' && window.config && window.config.apiUri
  ? window.config.apiUri : 'https://api-dev.floq.no';

const apiToken = typeof window !== 'undefined' && window.apiToken
...
export const getProjects = () => fetch(`${baseURL}/projects?select=id,name,billable,customer,responsible,active&order=id.desc`, {
  headers
...
export const getEmployees = () => fetch(`${baseURL}/employees?select=id,first_name,last_name`, {
  headers
}).then(response => response.json());
