...
export default (
  state = { loading: true, data: new Immutable.Map(), excludeInactiveProjects: true },
  action
) => {
  switch (action.type) {
    case FETCH_PROJECTS:
      return {
        ...state,
        loading: false,
        data: new Immutable.OrderedMap(action.payload.map(e => [e.id, e])).sortBy(p => p.id.toLowerCase())
      };
    case CREATE_PROJECT:
      return {
        ...state,
        loading: false,
        data: state.data.set(action.payload.id, action.payload).sortBy(p => p.id.toLowerCase())
      };
    case UPDATE_PROJECT:
      return {
        ...state,
